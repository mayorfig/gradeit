...
def responses(request):
    essay_list = Essay.objects.select_related("question").all().order_by("-created_at")
...
def question(request, question_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            matric_no = form.cleaned_data.get("matric")
            name = form.cleaned_data.get("name")

            if len(content) > 20:
                num_features = 300
                model = word2vec.KeyedVectors.load_word2vec_format(
                    os.path.join(current_path, "deep_learning_files/word2vec.bin"),
                    binary=True,
                )
                clean_test_essays = []
                ...
                lstm_model.load_weights(
                    os.path.join(current_path, "deep_learning_files/final_lstm.h5")
                )
